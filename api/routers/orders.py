...
def create_order(order: OrderIn, repo: OrderRepo = Depends(), candy_repo: CandyRepository = Depends()):
    return repo.create(order, candy_repo)


@router.get("/orders", response_model=Union[List[OrderOut], Error])
def get_all_orders(repo: OrderRepo = Depends(), candy_repo: CandyRepository = Depends()):
...
def get_one_order(
    response: Response, order_id: int, repo: OrderRepo = Depends(), candy_repo: CandyRepository = Depends()
) -> OrderOut:
...
def update_order(order_id: int, order: OrderIn, repo: OrderRepo = Depends(), candy_repo: CandyRepository = Depends()):
